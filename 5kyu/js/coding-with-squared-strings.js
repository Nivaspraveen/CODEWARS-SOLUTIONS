...
// Topics: Fundamentals, Strings, Ciphers, Cryptography

function code(s) {
    let n = Math.ceil(Math.sqrt(s.length));
    s = s.padEnd(n * n, '\v');
    let grid = [];
    for (let i = 0; i < n; i++) grid.push(s.slice(i * n, (i + 1) * n));
    let rotated = Array.from({ length: n }, (_, i) => grid.map(r => r[i]).reverse().join(''));
    return rotated.join('\n');
}
  
function decode(s) {
    let grid = s.split('\n'), n = grid.length, 
        rotated = Array.from({ length: n }, (_, i) => grid.map(r => r[n - i - 1]).join('')),
        result = rotated.join('');
    return result.replace(/\v/g, '');
}