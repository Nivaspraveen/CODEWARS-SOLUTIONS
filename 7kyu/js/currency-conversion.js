...
function convertMyDollars(usd, currency) {
    const currencyStartsWithVowel = ['a', 'e', 'i', 'o', 'u'].includes(currency[0].toLowerCase());
    let conversionRate = CONVERSION_RATES[currency];

    if (!currencyStartsWithVowel) conversionRate = parseInt(conversionRate.toString(), 2);
    
    const foreignCurrencyAmount = usd * conversionRate;
    return `You now have ${foreignCurrencyAmount} of ${currency}.`;
}