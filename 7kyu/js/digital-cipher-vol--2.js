...
function decode  (code, n) {
    const digits = Array.from(String(n), num => parseInt(num));
    const decodedMsg = code.map((num, idx) => {
      const digit = digits[idx % digits.length], 
            letterValue = (num - digit - 1 + 26) % 26 + 1;
      return String.fromCharCode(letterValue + 96);
    });
