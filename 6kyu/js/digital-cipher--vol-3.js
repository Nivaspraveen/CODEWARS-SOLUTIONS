...
function findTheKey(message, code) {
    const letterValues = message.split('').map(char => char.charCodeAt(0) - 96),
          keyPattern = code.map((num, i) => num - letterValues[i]),
          n = keyPattern.length;
    for (let l = 1; l <= n; l++) {
      const candidate = keyPattern.slice(0, l);
      let repeated = [];
      for (let i = 0; i < n; i++) repeated.push(candidate[i % l]);
      if (JSON.stringify(repeated) === JSON.stringify(keyPattern)) return parseInt(candidate.join(''));
    }
}