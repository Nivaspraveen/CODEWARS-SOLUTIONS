...
// Topics: Arrays, Strings, Ciphers

function decipher(code) {
    const charToValue = char => (char === ' ' ? 0 : char.charCodeAt(0) - 'a'.charCodeAt(0) + 1),
            valueToChar = value => (value === 0 ? ' ' : String.fromCharCode(value - 1 + 'a'.charCodeAt(0))),
            numCodes = code.length, 
            codeLength = code[0].length,
            sum = Array(codeLength).fill(0);
    code.forEach(c => {
      for (let i = 0; i < codeLength; i++) sum[i] += charToValue(c[i]);
    });
    const result = sum.map(s => {
      const avg = Math.floor(s / numCodes);
      return valueToChar(avg);
    });
    return result.join('');
}