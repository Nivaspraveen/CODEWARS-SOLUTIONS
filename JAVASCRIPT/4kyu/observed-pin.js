...
function getPINs(observed) {
    ...
    const getCombinations = (curr, index) => {
      if (index === observed.length) return [curr];
      const digit = observed[index], neighbors = adj[digit], result = [];
      for (const neighbor of neighbors) result.push(...getCombinations(curr + neighbor, index + 1));
      return result;
    };
