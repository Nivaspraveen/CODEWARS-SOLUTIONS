...
// Probability, Mathematics, Fundamentals

function probability(length, num){
    const start = length === 1 ? 0 : 10 ** (length - 1), 
          end = 10 ** length - 1;
    const firstMul = Math.ceil(start / num) * num, 
          lastMul = Math.floor(end / num) * num;
    if (firstMul > end || lastMul < start) return 0;
    const countMuls = Math.floor((lastMul - firstMul) / num) + 1,
          totalNumbers = end - start + 1,
          prob = (countMuls / totalNumbers) * 100;
    return prob;
}